...
class MProduct(db.Model):
    ...
    brand_id = db.Column(db.Integer, db.ForeignKey('marketplace_brands.id', ondelete="CASCADE"))
    is_featured = db.Column(db.Boolean, default=False)
    ...
    @property
    def rating(self):
        return db.session.query(db.func.avg(MReview.score)).filter(MReview.product_id==self.id).scalar()
...
class MCartItem(db.Model):
    ...
    variant = db.relationship("MVariant", backref="variant_cart_items", primaryjoin="MVariant.id == MCartItem.variant_id")
...
class MOrderItem(db.Model):
    ...
    variant = db.relationship("MVariant", backref="variant_order_items", primaryjoin="MVariant.id == MOrderItem.variant_id")
...
class MReview(db.Model):
    ...
    product_id = db.Column(db.Integer, db.ForeignKey('marketplace_products.id', ondelete="CASCADE"), nullable=True)
    ...
    product = db.relationship("MProduct", cascade="all,delete", backref="reviews")
