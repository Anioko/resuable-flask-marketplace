...
def review():
    ...
    review = MReview(message=review_text, score=review_rating, user=current_user, product=product_instance)
...
def category(page, category_id, category_name):
    products = MProduct.query.join(MProductCategory).filter(MProductCategory.category_id==category_id).paginate(page, per_page=8)
    website_settings = MSettings.query.first()
...
def product(product_id, product_name):
    product_instance = db.session.query(MProduct).filter(MProduct.id == product_id).options(joinedload(MProduct.variants)).first()
    product_variants = product_instance.variants
    review_form = ReviewForm()
    # print(current_user.name)
    if current_user.is_authenticated:
        current_user_review = MReview.query.filter_by(user=current_user).filter_by(product=product_instance).first()
    website_settings = MSettings.query.first()
...
def seller_product_delete(product_id):
    website_settings = MSettings.query.first()
    cat = MProduct.query.filter_by(seller=current_user).filter_by(id=product_id).first()
    if not cat:
        abort(404)
...
def seller_variant_edit(variant_id):
    ...
    if request.method == 'POST':
        if form.validate_on_submit():
            ...
            db.session.commit()
            flash('Variant {} successfully Updated'.format(variant_instance.name), 'success')
            return redirect(url_for('marketplace.seller_variants'))
