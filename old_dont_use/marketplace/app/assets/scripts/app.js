...
$(document).ready(function () {
    ...
    $('#open-nav').on('click', function () {
        $('.mobile.only .vertical.menu:first').transition('slide down');
    });
    $('#open-nav1').on('click', function () {
        $('.mobile.only .vertical.menu:last').transition('slide down');
    });

    // Enable sortable tables
    if ($('table.ui.sortable').length){
